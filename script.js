...
let tasks = []; // { id, title, dueDate, priority, recurring, completed }
...
function addTaskFromModal() {
  ...
  saveTasks();
  renderAllSections();
  closeAddTaskModal(); // ✅ modal closes properly
}

// Rendering
function renderAllSections() {
  [todayList, completedList].forEach(ul => {
    if (ul) ul.innerHTML = "";
  });
  ...
  // Use safeAppend to avoid null errors
  inbox.forEach(t => safeAppend(inboxList, taskCard(t)));
  todayItems.forEach(t => safeAppend(todayList, taskCard(t)));
  upcoming.forEach(t => safeAppend(upcomingList, taskCard(t)));
  overdue.forEach(t => safeAppend(overdueList, taskCard(t)));
  completed.forEach(t => safeAppend(completedList, taskCard(t)));
...
function safeAppend(listEl, node) {
